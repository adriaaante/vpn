...
Одной командой (и на маке, и на сервере) — полный отчёт с отпечатками ключей,
который безопасно пересылать (секреты маскируются):

```bash
bash scripts/vpn-doctor.sh
```

Дальше — то же самое вручную:
