...
## 10. Авто-режим с `default selective` пугает пользователя
После установки тулбара авто-режим по сети при `default selective` сам возвращал
режим в selective → обычные сайты показывали RU. Пользователь воспринял это как
«утечку»/баг. Фиксы: в `configs/netmap.example` сделать `default full`
(безопасный дефолт — на незнакомой сети весь трафик через Латвию). Если нужен
«весь трафик всегда» — проще выключить авто-режим:
