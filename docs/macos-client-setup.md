...
## Выбор протокола (Reality / Hysteria2 / Авто)

По умолчанию активен режим **Авто** (`urltest`): клиент сам выбирает самый
быстрый рабочий протокол и переключается при блокировках. Но можно зафиксировать
конкретный — например, Hysteria2 для скорости или Reality для максимальной
скрытности.

Переключение **на лету, без перезапуска и без sudo** (через локальный Clash API
sing-box на `127.0.0.1:9090`):
```bash
bash scripts/vpn-proto.sh auto        # автоматический выбор (по умолчанию)
bash scripts/vpn-proto.sh reality     # VLESS+Reality (TCP, скрытный)
bash scripts/vpn-proto.sh hysteria2   # Hysteria2 (UDP, быстрый)
...
### Уведомления при переключении на резерв
Чтобы получать системное уведомление, когда клиент **сам** ушёл на резервный
протокол (например, Reality придавили и активным стал Hysteria2):
```bash
bash scripts/install-watcher.sh
```
Это ставит лёгкий LaunchAgent, который раз в ~20 сек сверяет активный протокол и
шлёт уведомление «было → стало». Ручные переключения не беспокоят. Работает без
sudo. Снять: `launchctl bootout gui/$(id -u) ~/Library/LaunchAgents/com.user.singbox-watch.plist`.

Этот же watcher делает **health-check**: если sing-box завис (процесс жив, но
трафик не идёт), после ~3 неудачных проб подряд (≈60 сек) он автоматически
перезапускает демон и шлёт уведомление «sing-box завис — перезапущен». Плюс
поддерживает актуальным список `utun` для kill-switch.
