...
### Фоновые службы и автозапуск (macOS)
В Системных настройках → Объекты входа есть три записи — все НАШИ, посторонних нет:
- **«Открывать при входе» → SwiftBar** (приложение) — добавлен как login item
  (`osascript ... make login item`), чтобы пульт в строке меню стартовал сам.
- **«Фоновая активность» → два «bash»** (помечены «неподтверждённый разработчик» —
  это норма для самодельных shell-скриптов, НЕ угроза):
  1. `com.user.singbox` (LaunchDaemon, `/Library/LaunchDaemons/`) — запускает
     `/bin/bash /etc/sing-box/daemon-run.sh` → поднимает kill-switch и сам туннель.
  2. `com.user.singbox-watch` (LaunchAgent, `~/Library/LaunchAgents/`) — запускает
     `/bin/bash …/scripts/vpn-watch.sh` → следит за здоровьем туннеля (раз в 20 с).
  ОБА должны быть ВКЛЮЧЕНЫ. Отображаются как «bash», т.к. запускаются через
  интерпретатор. Переименование (запуск скриптов напрямую под осмысленным именем)
  обсуждалось и СОЗНАТЕЛЬНО НЕ делалось: чисто косметика, требует пересборки служб
  и риска для рабочего автозапуска; результат на разных версиях macOS не гарантирован.

### Модель угроз: что если сервер скомпрометируют
Сервер — это ИСХОДЯЩИЙ релей (Mac сам подключается к нему), у него НЕТ обратного
доступа в Mac. Захват сервера НЕ даёт доступа к файлам/приложениям Mac. Банковский
трафик защищён сквозным TLS (+ pinning) — сервер-MITM не расшифрует содержимое,
видит только метаданные (куда шёл туннелированный трафик). В режиме `full` RU-сайты
и банки идут НАПРЯМУЮ, минуя сервер вообще. Единственный теор. риск — эксплойт
уязвимости в КЛИЕНТЕ sing-box со стороны злого сервера (низкий; митигируется
`brew upgrade sing-box`). На сервере хранятся Reality private key + UUID — их захват
позволяет лишь использовать/подменять туннель, но не лезть в Mac; при подозрении —
ротация ключей на сервере.
