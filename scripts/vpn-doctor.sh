...
if [[ "$MODE" == "client" ]]; then
  ...
  # Туннель мог быть ВЫКЛЮЧЕН сторожем (3 неудачных рестарта подряд). Снаружи это
  # выглядит как «VPN не работает» и легко уводит в поиск блокировки, хотя лечится
  # одной кнопкой. Метку ставит vpn-watch.sh, снимает успешный подъём и `vpn off`.
  AUTOOFF="$HOME/.cache/vpn-auto-off"
  if [[ -f "$AUTOOFF" ]]; then
    off_at="$(cat "$AUTOOFF" 2>/dev/null || echo 0)"
    if [[ "$off_at" =~ ^[0-9]+$ ]]; then
      mins=$(( ( $(date +%s) - off_at ) / 60 ))
      echo "  ⚠️  туннель ВЫКЛЮЧЕН аварийно $mins мин. назад (сторож: 3 рестарта не помогли)."
      echo "      Это НЕ обязательно блокировка. Сторож сам повторит попытку, включить сразу: vpn on"
    fi
  fi
