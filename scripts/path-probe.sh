...
echo "  (на них никто не слушает: 'refused' = SYN/RST дошли и вернулись; 'timeout' = режут"
echo "   и там. НО: см. вердикт ниже — при блокировке всего IP refused тоже приходит)"
...
LISTEN_DEAD=0; LISTEN_OK=0; CLEAN=0
for p in $ALT_PORTS; do
  st="$(port_state "$p")"
  printf '  порт %-6s %s\n' "$p" "$st"
  case "$st" in
    *"TLS ПРОХОДИТ"*) LISTEN_OK=$((LISTEN_OK+1)) ;;
    *"TLS режут"*)    LISTEN_DEAD=$((LISTEN_DEAD+1)) ;;
    *"путь чист"*)    CLEAN=$((CLEAN+1)) ;;
  esac
done
# Грабля №5c (2026-08-20 и 2026-09-02): порты С живым слушателем мертвы, а «чистые»
# без слушателя отвечают refused — потому что RST пропускают, а любой пакет
# С ДАННЫМИ к этому IP выбрасывают. Дважды на это купились и поднимали слушателя
# на «чистом» порту — он умирал так же. Поэтому ✅ выше в этой ситуации ложный.
if (( LISTEN_DEAD > 0 && LISTEN_OK == 0 && CLEAN > 0 )); then
  echo
  echo "  ⚠️ ВЕРДИКТ: режут ВЕСЬ IP, а не порт (грабля №5c)."
  echo "     Слушающие порты мертвы, а refused на пустых — лишь потому, что RST пропускают,"
  echo "     а пакеты с данными к этому IP выбрасывают. Слушатель на «чистом» порту умрёт"
  echo "     так же. Перенос порта НЕ поможет — нужен новый IP:"
  echo "       сервер: bash scripts/change-server-ip.sh <новый-IP>   (через VNC EDIS)"
  echo "       мак:    bash scripts/set-server-ip.sh <новый-IP>"
fi
